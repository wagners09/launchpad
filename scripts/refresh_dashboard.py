#!/usr/bin/env python3
"""
Pulls the count of eBay orders that are sold but not yet shipped, and
rewrites the "Ready to ship" tile + card in index.html between the
RTS_TILE_START/END and RTS_CARD_START/END HTML comment markers.
...
def get_ebay_count():
    ...
    # "Ready to ship" should actually mean. (Filtering by creation date
    # instead would keep counting an order after you've already shipped it,
...
def build_card(ebay_count):
    if ebay_count > 0:
        noun = "order" if ebay_count == 1 else "orders"
        rows = (
            ...
            f'        <p class="dash-item-meta">{ebay_count} {noun} awaiting shipment</p>\n'
            "      </div>\n"
            "    </a>"
        )
    else:
        rows = '    <div class="dash-empty">Nothing waiting to ship on eBay.</div>'

    return (
        '\n  <div class="dash-section-label">Ready to ship &middot; eBay</div>\n'
...
def main():
    ...
    print(f"eBay: {ebay_count} order(s) awaiting shipment")
